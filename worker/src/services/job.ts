...
export default class JobService implements CrudService {
    ...
    public async getSchedules(): Promise<Array<JobSchedulerDto>> {
        const schedules: JobSchedulerDto[] = await JobScheduler.findAll({ where: { status: {[Op.in]: [JobStatus.READY, JobStatus.SCHEDULED] }, nextExecutionTime : {[Op.lte]: DateTimeUtility.getCurrentDateTime()} } });
        return <JobSchedulerDto[]>schedules;
    }
}
